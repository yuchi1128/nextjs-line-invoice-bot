              ...
              <div className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300">
                <div className="p-3 flex items-center justify-between">
                  <div className="flex-shrink-0 w-1/4">
                    <p className="text-xs text-gray-600 mb-1">{new Date(invoice.sentDate).toLocaleDateString()}</p>
                    <h2 className="text-sm font-semibold text-gray-800 truncate">{invoice.recipient}</h2>
                  </div>
                  <div className="flex-grow text-center">
                    <p className="text-lg font-bold text-gray-700">{invoice.amount.toLocaleString()}円</p>
                  </div>
                  <div className="flex-shrink-0 w-1/4 text-right">
                    <span className={`px-2 py-1 rounded-full text-sm font-semibold ${invoice.isPaid ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                      {invoice.isPaid ? '精算済' : '未精算'}
                    </span>
                  </div>
                </div>
              </div>
